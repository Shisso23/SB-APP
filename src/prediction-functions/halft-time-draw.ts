...
export const predictHTDraw = ({
  ...
  const predictedFixtures = currentFixtures.filter((currentFixture) => {
    ...
    return (
      ((homeTeamStanding?.all.goals.for / homeTeamStanding?.all.played <=
        0.85 &&
        awayTeamStanding?.all.goals.for / awayTeamStanding?.all.played <=
          0.85) ||
        (againstHomeTeamGoalsPercentage({ homeTeamStanding }) < 85 &&
  ...
  return {
    ...
    option: betOptions.find(
      (option) => option.id === betOptionsEnum.HT_DRAW
    ) as betOptionModel,
