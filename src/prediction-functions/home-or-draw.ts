...
export const predictHomeOrDraw = ({
  ...
  const predictedFixtures = currentFixtures.filter((currentFixture) => {
    ...
    return (
      ...
      Math.abs(homeTeamStanding?.rank - awayTeamStanding?.rank) > 3 &&
      head2HeadMatches.every(
        (match) =>
          (match.goals.home >= match.goals.away &&
            match.teams.home.id === currentFixture.teams.home.id) ||
          (match.goals.away >= match.goals.home &&
            match.teams.away.id === currentFixture.teams.home.id)
      )
    );
  });
  return {
