...
export const predictBothTeamsToScore = ({
  ...
  const predictedFixtures = currentFixtures.filter(currentFixture => {
    ...
    return (awayTeamScroreInMostAwayFixtures({
        ...
        minGoals: 1,
      }) && (homeTeamStanding.rank> awayTeamStanding.rank ) && (homeTeamStanding.rank - awayTeamStanding.rank <=10) && againstAwayTeamGoalsPercentage({awayTeamStanding}) >=120 && homeTeamGoalsPercentage({homeTeamStanding})>= 140
    );
  });
  return {
    fixtures: predictedFixtures,
  ...
  const predictedFixtures = currentFixtures.filter(currentFixture => {
    ...
    if (lastFiveHomeTeamHomeFixtures.length >= 3) {
      return (
        ...
        againstHomeTeamGoalsPercentage({ homeTeamStanding }) <= 150 && awayTeamFailWinningInMostAwayFixtures({awayFixtures: lastFiveAwayTeamAwayFixtures})
      );
    }
    return false;
  });
...
export const predictMultiGoals0_3 = ({
  ...
  const predictedFixtures = currentFixtures.filter((currentFixture) => {
    ...
    return (
      ( ( homeTeamStanding.all.goals.for/ homeTeamStanding.all.played ) <= 0.9 &&  (awayTeamStanding.all.goals.for/  awayTeamStanding.all.played )<=0.9)||
      againstHomeTeamGoalsPercentage({ homeTeamStanding }) < 90 && againstAwayTeamGoalsPercentage({ awayTeamStanding }) < 90
    );
  });
  return {
    fixtures: predictedFixtures,
