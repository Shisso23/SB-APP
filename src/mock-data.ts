/* eslint-disable @typescript-eslint/ban-types */
import { FixtureDataModel } from "./models/fixtures";
import { LeagueDataModel, LeagueDataSeasonsModel } from "./models/leagues";
import { StandingsModel } from "./models/standings-models";


export const favLeaguesMock: LeagueDataModel[] = [
  new LeagueDataModel({
    country: {
      code: 'ABC',
      flag: 'NONE',
      name: 'Italy',
    },
    league: {
      id: 39,
      logo: 'er',
      name: 'Serie a',
      type: 'any',
      selected: 'false',
    },
    seasons: [
      new LeagueDataSeasonsModel({
        current: true,
        end: '12/11/2023',
        start: '12/11/2021',
        year: 2023,
      }),
    ],
  }),
  new LeagueDataModel({
    country: {
      code: 'ABC',
      flag: 'NONE',
      name: 'Italy',
    },
    league: {
      id: 39,
      logo: 'er',
      name: 'Serie a',
      type: 'any',
      selected: 'false',
    },
    seasons: [
      new LeagueDataSeasonsModel({
        current: true,
        end: '12/11/2023',
        start: '12/11/2021',
        year: 2023,
      }),
    ],
  })
];

export const mockFixtures: FixtureDataModel[]= [
  new FixtureDataModel({
    fixture: {
      date: new Date().toString(),
      id: 874877,
      timezone: 'UTC',
      status: {long: 'Not Started', short: 'NS', elapsed: 0},
      timestamp: 166826520,
    },
    goals: {home: 0, away: 0},
    league: {
      country: 'Belgium',
      // flag: 'https://media.api-sports.io/flags/be.svg',
      id: 144,
      logo: 'https://media.api-sports.io/football/leagues/144.png',
      name: 'Jupiler Pro League',
      // round: 'Regular Season - 17',
      season: 2023,
    },
    score: {
      extratime: {home: 0, away: 0},
      fulltime: {home: 0, away: 0},
      halftime: {home: 0, away: 0},
    },
    teams: {
      away: {
        id: 624,
        logo: 'https://media.api-sports.io/football/teams/624.png',
        name: 'Oostende',
        winner: true,
      },
      home: {
        id: 261,
        name: 'KVC Westerlo',
        logo: 'https://media.api-sports.io/football/teams/261.png',
        winner: false,
      },
    },
  }),
  new FixtureDataModel({
    fixture: {
      date: new Date().toString(),
      id: 874878,
      timezone: 'UTC',
      status: {long: 'Not Started', short: 'NS', elapsed: 0},
      timestamp: 166826520,
    },
    goals: {home: 0, away: 0},
    league: {
      country: 'Belgium',
      // flag: 'https://media.api-sports.io/flags/be.svg',
      id: 144,
      logo: 'https://media.api-sports.io/football/leagues/144.png',
      name: 'Jupiler Pro League',
      // round: 'Regular Season - 17',
      season: 2023,
    },
    score: {
      extratime: {home: 0, away: 0},
      fulltime: {home: 0, away: 0},
      halftime: {home: 0, away: 0},
    },
    teams: {
      away: {
        id: 624,
        logo: 'https://media.api-sports.io/football/teams/624.png',
        name: 'Oostende',
        winner: true,
      },
      home: {
        id: 261,
        name: 'KVC Westerlo',
        logo: 'https://media.api-sports.io/football/teams/261.png',
        winner: false,
      },
    },
  }),
];

export const MockLeaguesStandings :StandingsModel[]=[]


export const goupedFixturesMock: {
  fixtures: FixtureDataModel[];
  option: {
    name: string;
    id: number;
    level: number;
    shortName: string;
    description: string;
  };
}[] = new Array(20).fill( // Change the number inside to generate more or less
  {
    fixtures: [
      new FixtureDataModel({
        fixture: {
          date: new Date().toString(),
          id: 874879,
          timezone: 'UTC',
          status: {long: 'Not Started', short: 'NS', elapsed: 0},
          timestamp: 166826520,
        },
        goals: {home: 0, away: 0},
        league: {
          country: 'Belgium',
          // flag: 'https://media.api-sports.io/flags/be.svg',
          id: 144,
          logo: 'https://media.api-sports.io/football/leagues/144.png',
          name: 'Jupiler Pro League',
          // round: 'Regular Season - 17',
          season: 2022,
        },
        score: {
          extratime: {home: 0, away: 0},
          fulltime: {home: 0, away: 0},
          halftime: {home: 0, away: 0},
        },
        teams: {
          away: {
            id: 624,
            logo: 'https://media.api-sports.io/football/teams/624.png',
            name: 'Oostende',
            winner: true,
          },
          home: {
            id: 261,
            name: 'KVC Westerlo',
            logo: 'https://media.api-sports.io/football/teams/261.png',
            winner: false,
          },
        },
      }),
    ],
    option: {
      id: 3,
      level: 3,
      description: 'Both teams to score',
      name: 'BTTS',
      shortName: 'BTTS',
    },
  }
) ;
